...
function push(added) {
  ...
  git(['add', 'scripts/agent/inbox.json', 'scripts/agent/articles'], (e1, o1) => {
    ...
    git(['-c', 'user.name=Avare Collector', '-c', 'user.email=olianayda@gmail.com', 'commit', '-q', '-m', msg], (e2, o2) => {
      ...
      // autostash — чтобы незакоммиченные правки очереди из панели не мешали.
      git(['pull', '--rebase', '--autostash', '-q', 'origin', 'main'], (e3, o3) => {
        if (e3) return log(`подтянуть не удалось, пуш пропущен: ${o3.trim()}`);
        git(['push', '-q', 'origin', 'main'], (e4, o4) =>
          log(e4 ? `пуш не прошёл: ${o4.trim()}` : 'входящие отправлены в репозиторий')
        );
      });
    });
  });
